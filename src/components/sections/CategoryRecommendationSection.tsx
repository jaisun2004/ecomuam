    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex-1 space-y-4">
              <p className="text-sm text-muted-foreground leading-relaxed">
                Analysis of search queries and competitor listings suggests a gap in the market for <span className="text-foreground font-bold">Spicy/Tangy Fruit Blends</span>.
                "Mango Chilli" has seen a <span className="text-success font-bold">+45%</span> increase in search volume over the last quarter with only 2 niche competitors.
              </p>
              <div className="p-3 bg-info/5 border border-info/20 rounded-lg">
                ...
                <p className="text-xs text-foreground">Launch "Mango Chilli" 250ml Can at ₹50 price point to capture early adopters.</p>
              </div>
            </div>
          </div>
        </div>
            ...
            {flavorOpportunities.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between">
                ...
                <span className={`text-xs font-bold w-12 text-right ${
                  item.potential === 'High' ? 'text-success' :
                  item.potential === 'Medium' ? 'text-warning' : 'text-muted-foreground'
                }`}>
                  {item.potential}
                </span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border bg-card shadow-card p-5">
          ...
          <p className="text-xs text-muted-foreground mb-4">Which formats are growing? (YoY Growth Rate %)</p>
            ...
            {packSizeOpportunities.map((item, idx) => (
              <div key={idx}>
                ...
                <div className="h-3 bg-muted rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${item.growth > 0 ? 'bg-success' : 'bg-destructive'}`}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-border text-xs text-muted-foreground uppercase bg-muted/50">
                  ...
                  <th className="p-3 text-center font-bold">Recommendation</th>
                </tr>
              </thead>
